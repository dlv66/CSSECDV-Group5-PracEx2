...
import { getUserFromToken, JwtPayload } from "@/lib/utils/jwt";
import { userHasPermission, getUserPermissions } from "@/lib/roles/permission";

// Standardized error responses
export function handleAuthorizationError(errorType: 'unauthenticated' | 'unauthorized' | 'server_error') {
    const responses = {
...
export function requireRole(allowedRoles: string[]) {
    return async (): Promise<{ user: JwtPayload; error?: NextResponse }> => {
        try {
            // Check if user is authenticated
            const user = await getUserFromToken();
            if (!user) {
                return { user: null as any, error: handleAuthorizationError('unauthenticated') };
            }
            ...
            if (!hasPermission) {
                return { user, error: handleAuthorizationError('unauthorized') };
            }

            return { user, error: undefined };
        } catch (error) {
            console.error('Role authorization check failed:', error);
            return { user: null as any, error: handleAuthorizationError('server_error') };
        }
    };
}
...
export async function withRoleAuthorization(allowedRoles: string[]) {
    const authCheck = requireRole(allowedRoles);
    return await authCheck();
}

export async function withPermissionAuthorization(requiredPermission: string) {
